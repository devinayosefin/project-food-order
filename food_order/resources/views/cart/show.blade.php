...
@section('content')
    <div class="p-8 bg-white rounded-lg shadow-md">
        ...
        @endif
        @php
            $validCart = collect($cart ?? [])->filter(fn($item) => isset($item['menu_id']) && $item['quantity'] > 0);
        @endphp
        ...
        @else
                        ...
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            ...
                            <td class="py-3 px-6 text-left">
                                <form action="{{ route('cart.update') }}" method="POST" class="flex items-center space-x-2">
                                    ...
                                    <button type="submit" name="action" value="decrement" class="bg-gray-300 px-2 py-1 rounded hover:bg-gray-400 font-bold text-lg">-</button>
                                    <span class="mx-2">{{ $item['quantity'] }}</span>
                                    <button type="submit" name="action" value="increment" class="bg-gray-300 px-2 py-1 rounded hover:bg-gray-400 font-bold text-lg">+</button>
                                </form>
                            </td>
            ...
            <div class="text-right font-bold text-xl mb-4">
                Harga Total: Rp{{ number_format($validCart->sum(fn($item) => $item['quantity'] * $item['price']), 0, ',', '.') }}
            </div>
